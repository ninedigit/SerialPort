...
namespace NineDigit.SerialPort.Platforms.Windows
{
    ...
    public sealed class Win32DeviceManagement
    {
        #region Dll import
        ...
        private static extern bool SetupDiEnumDeviceInfo(IntPtr DeviceInfoSet, UInt32 MemberIndex,
            ref SP_DEVINFO_DATA DeviceInterfaceData);

        [DllImport("setupapi.dll", SetLastError = true)]
        private static extern IntPtr SetupDiGetClassDevs(ref Guid gClass, UInt32 iEnumerator, UInt32 hParent,
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Globalization",
            "CA2101:Specify marshaling for P/Invoke string arguments", Justification = "<Pending>")]
        [DllImport("setupapi.dll", SetLastError = true)]
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Performance",
            "CA1810:Initialize reference type static fields inline", Justification = "<Pending>")]
        static Win32DeviceManagement()
        {
        ...
        public static IReadOnlyCollection<SerialDeviceInfo> GetSerialDevices()
        {
            ...
            for (int index = 0; index < guids.Length; index++)
            {
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        bool success = SetupDiEnumDeviceInfo(hDeviceInfoSet, memberIndex, ref deviceInfoData);
                        if (!success)
                        {
                        ...
                        var deviceInfo = new SerialDeviceInfo(
                            portName: GetDeviceName(hDeviceInfoSet, deviceInfoData),
                            description: GetDeviceDescription(hDeviceInfoSet, deviceInfoData),
                            busDescription: GetDeviceBusDescription(hDeviceInfoSet, deviceInfoData));
        ...
        {
            IntPtr hDeviceRegistryKey = SetupDiOpenDevRegKey(
                pDevInfoSet, ref deviceInfoData,
            ...
            try
            {
                int result = RegQueryValueEx(hDeviceRegistryKey, "PortName", 0, out uint lpRegKeyType, ptrBuf,
                    ref length);

                if (result != 0)
                    throw new Exception(
                        $"Can not read registry value PortName for device '{deviceInfoData.ClassGuid}'. Return code: {result}.");
            }
            finally
            {
        ...
        private static string GetDeviceDescription(IntPtr hDeviceInfoSet, SP_DEVINFO_DATA deviceInfoData)
        {
            ...
            var success = SetupDiGetDeviceRegistryProperty(
                hDeviceInfoSet, ref deviceInfoData, SPDRP.SPDRP_DEVICEDESC,
                out _, ptrBuf, BUFFER_SIZE, out uint RequiredSize);

            if (!success)
            {
                var win32errorCode = Marshal.GetLastWin32Error();
                throw new Exception(
                    $"Can not read registry value PortName for device '{deviceInfoData.ClassGuid}'. Error code: {win32errorCode}.");
            }
        ...
        private static string GetDeviceBusDescription(IntPtr hDeviceInfoSet, SP_DEVINFO_DATA deviceInfoData)
        {
            ...
            var success = SetupDiGetDevicePropertyW(
                hDeviceInfoSet, ref deviceInfoData, ref DEVPKEY_Device_BusReportedDeviceDesc,
                out uint _, ptrBuf, BUFFER_SIZE, out uint RequiredSize, 0);

            if (!success)
            {
                var win32errorCode = Marshal.GetLastWin32Error();

                if (win32errorCode == ERROR_ELEMENT_NOT_FOUND)
                    return null;

                throw new Exception(
                    $"Can not read Bus provided device description for device '{deviceInfoData.ClassGuid}'. Error code: {win32errorCode}.");
            }
        ...
        private static Guid[] GetClassGUIDs(string className)
        {
            ...
            if (SetupDiClassGuidsFromName(className, ref guidArray[0], 1, out uint requiredSize))
            {
                if (requiredSize > 1)
                {
                    guidArray = new Guid[requiredSize];
                    SetupDiClassGuidsFromName(className, ref guidArray[0], requiredSize, out _);
                }
            }
            else
                throw new Win32Exception("SetupDiClassGuidsFromName failed.");
